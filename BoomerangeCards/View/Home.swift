...
struct Home: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            BoomerangCard(isRotationEnabled: isRotationEnabled, isBlurEabled: isRotationEnabled, cards: $cards)
                .frame(height: 200)
    ...
    func setupCards(){
        for index in 1...8 {
            cards.append(.init(imageName: "p\(index)"))
        }
...
struct BoomerangCard: View {
    ...
    func CardView(card: Card, size: CGSize) -> some View {
        ...
        Image(card.imageName)
            ...
            .frame(width: size.width, height: size.height)
            .blur(radius: card.isRotated && isBlurEabled ? 6.5 : 0)
